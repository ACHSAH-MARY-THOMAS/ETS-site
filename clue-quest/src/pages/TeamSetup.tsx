...
const TeamSetup = () => {
    ...
    useEffect(() => {
        ...
        if (agent1.trim() && agent1.length < 2) {
            newErrors.agent1 = "Agent 1 name must be at least 2 characters";
        }
        if (agent2.trim() && agent2.length < 2) {
            newErrors.agent2 = "Agent 2 name must be at least 2 characters";
        }
        
        setErrors(newErrors);
    }, [teamName, agent1, agent2]);

    const isFormValid = teamName.trim() && agent1.trim() && agent2.trim() && Object.keys(errors).length === 0;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Store team data
        const teamData = {
            teamId: teamName.toUpperCase().replace(/\s+/g, "_"),
        ...
        try {
            ...
            sessionStorage.setItem("lockstep_team", JSON.stringify(teamData));
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        >
                            ...
                            >
                                <Input
                                    type="text"
                                    placeholder="e.g., Shadow Ops"
                                    value={teamName}
                                    onChange={(e) => setTeamName(e.target.value)}
                                    onFocus={() => setFocusedField("teamName")}
                                    onBlur={() => setFocusedField(null)}
                                    className={`bg-black/40 border-2 text-foreground placeholder:text-muted-foreground/50 text-xs md:text-sm focus:outline-none transition-all uppercase tracking-wide ${
                                        errors.teamName
                            ...
                            {[teamName, agent1, agent2].map((field, i) => (
                                <motion.div
                                    ...
                                    className={`h-2 w-8 rounded transition-all ${
                                        field.trim() && !errors[["teamName", "agent1", "agent2"][i]]
                                            ? "bg-green-500"
                                            : field.trim()
                                            ? "bg-red-500"
                        ...
                        >
                            <Button
                                ...
                                className={`w-full text-black text-xs md:text-sm font-['Press_Start_2P'] uppercase tracking-widest py-6 md:py-8 transition-all ${
                ...
                {showSuccess && (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <CheckCircle2 className="w-full h-full text-green-400 drop-shadow-[0_0_20px_rgba(34,197,94,0.6)]" />
                            </motion.div>
                            <motion.p
