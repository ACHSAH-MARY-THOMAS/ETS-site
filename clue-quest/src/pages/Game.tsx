...
interface Level {
    id: number;
    title: string;
    type?: "clock" | "url" | "crossword" | "morse" | "filesystem" | "text";
    content: string;
    hint?: string;
    targetTime?: string;
    directions?: string[];
...
const DEMO_LEVELS: Level[] = [
    ...
    },
    {
        id: 6,
        title: "Layer 06",
        content: "Placeholder puzzle 6\n\nThis is a placeholder for level 6.\nUpdate with actual puzzle content.",
    ...
    },
    {
        id: 7,
        title: "Layer 07",
        content: "Placeholder puzzle 7\n\nThis is a placeholder for level 7.\nUpdate with actual puzzle content.",
...
const Game = () => {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-['Press_Start_2P'] flex flex-col relative overflow-hidden">
            ...
            <main className="flex-1 flex flex-col md:flex-row gap-4 p-4 md:p-6 z-10 overflow-hidden">

                {/* Left Column: Security Layers */}
                <section className="w-full md:w-1/4 h-auto flex flex-col gap-4">
                    <div className="border-2 border-primary/10 bg-black/20 p-4 rounded backdrop-blur-sm h-[450px] md:h-[600px] flex flex-col">
                        <div className="text-[10px] text-primary/60 mb-4 border-b border-primary/10 pb-2 flex justify-between flex-shrink-0">
                            <span>SECURITY LAYERS</span>
                            <span>{currentLevel}/{DEMO_LEVELS.length}</span>
                        </div>

                        <div className="flex flex-col gap-2 overflow-y-auto flex-1 pr-2 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent">
                        {DEMO_LEVELS.map((layer) => (
                            <div
                                key={layer.id}
                                className={`flex items-center gap-3 p-3 border rounded text-[8px] transition-all duration-300 ${layer.id === currentLevel
                            ...
                            >
                                <div className={`w-2 h-2 rounded-full ${layer.id === currentLevel ? "bg-primary animate-pulse" : layer.id < currentLevel ? "bg-primary/40" : "bg-white/10"}`} />
                    ...
                    <div className="mt-auto pt-4 border-t border-primary/10 text-[8px] flex justify-between text-muted-foreground/50 flex-shrink-0">
                        <span>PROGRESS</span>
                ...
                <section className="flex-1 h-full min-h-[400px] relative border-4 border-primary/30 rounded-lg overflow-hidden flex flex-col shadow-2xl glow-primary">
                    ...
                    <div className="relative z-10 flex-1 flex flex-col p-6 md:p-8">
                        <div className="flex-1 flex flex-col gap-6">
                            ...
                            {currentPuzzle?.type !== "clock" && currentPuzzle?.type !== "url" && currentPuzzle?.type !== "crossword" && currentPuzzle?.type !== "morse" && currentPuzzle?.type !== "filesystem" && (
                ...
                <section className="w-full md:w-1/4 h-full flex flex-col gap-4 border-2 border-primary/10 bg-black/20 p-4 rounded backdrop-blur-sm">
